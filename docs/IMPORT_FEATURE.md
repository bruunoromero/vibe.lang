...
- Package-level imports rely on each dependency's `vibe.config.*` `package.sources` directory for resolution, and bare specifiers (e.g., `(import "@vibe/prelude")`) load the config's `package.entry` file by default while nested specifiers map to `.lang` files under that sources folder.

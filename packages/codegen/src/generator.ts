...
class ModuleEmitter {
  ...
  private readonly runtimeHelpers = new Map<string, string>();
  ...
  constructor(
    ...
    const hasKeywordNodes = graph.nodes.some(
  ...
  private emitImports(): void {
    ...
    if (this.runtimeHelpers.size > 0) {
      const specifiers = [...this.runtimeHelpers.entries()]
        ...
        .join(", ");
      this.addLine(`import { ${specifiers} } from '@vibe/runtime';`);
      emitted = true;
    }
    if (emitted) {
      this.addLine("");
